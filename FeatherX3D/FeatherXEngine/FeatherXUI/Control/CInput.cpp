...
,   m_bOnlyNumber(false), m_bSkipEnterKey(false)
,	m_bLimitWide(false), m_bNumberAndCapital(false)
...
const BOOL	CInput::IsLimit() const
{
	if(m_bLimitWide)
	{
		int nSize = (int)wcslen( m_input.c_str() );
	...
	{
		CHAR buff[1024] = {0,};
		int len = WideCharToMultiByte( m_codePage, 0, m_input.c_str(), (int)m_input.length(), buff, 1024, NULL, NULL );
...
{  
	if( m_bOnlyNumber || m_bNumberAndCapital)
		return true;
	...
	if(lParam&GCS_COMPSTR) {
		...
		if(hImc) { 
			...
			wchar_t* temp = (wchar_t*)_malloca(tempSize);
	...
	if(lParam&GCS_COMPATTR) {
		...
		if(hImc) { 
			...
			BYTE* temp = (BYTE*)_malloca(tempSize);
...
{  
	m_comp.resize(0);
	m_ulStart = m_ulEnd = 0;	m_reading.resize(0);
...
/*----------------------------------------------------------------------------*/ 
bool	CInput::OnChar(HWND hWnd, WPARAM wParam, LPARAM lParam, std::wstring& cut_text)
{  
	switch(wParam) 
	{
	case '\r':
...
const bool	CInput::IsEmpty() const
{
	return m_input.empty();
}

void CInput::AddInput( const wchar_t* pStr )
{ 
...
void CInput::SetInput( const wchar_t* pStr )
{  
	m_input = pStr;
}

/*----------------------------------------------------------------------------*/ 
int		CInput::GetInput(std::wstring& text )
{  
	text = m_input;
...
//{
//	return ConvertString(m_codePage, m_input.c_str(), (int)m_input.size(), text, len);
//}

/*----------------------------------------------------------------------------*/ 
int		CInput::GetComp(std::wstring& text )
{  
	text = m_comp;
	return (int)text.length();
}
...
//{
//	return ConvertString(m_codePage, m_comp.c_str(), (int)m_comp.size(), text, len);
//}

/*----------------------------------------------------------------------------*/ 
int	CInput::GetReading(std::wstring& text)
{  
	text = m_reading;
	return (int)text.length();
}
...
//{
//	return ConvertString(m_codePage, m_reading.c_str(), (int)m_reading.size(), text, len);
//}
	...
	if(m_candidate.empty()) {
		...
		if(index >= candidateList->dwCount) {
			...
			if(m_bUnicodeIME) {
				...
				return ConvertString(m_codePage, wText, (int)wcslen(wText), text, len);
...
{  
    ...
    if( _GetReadingString ) {
		...
		if( wstrLen == 0 ) {
			...
			wchar_t *wstr = (wchar_t*)_malloca(sizeof(wchar_t) * wstrLen);
		...
		INPUTCONTEXT *lpIC = CIMM::GetSingleton()->LockIMC(hImc);
		...
		if(lpIC == NULL) {
			...
			switch( m_dwId[0] )
			{
				...
				case IMEID_CHT_VER44: // New ChanJie IME98b          : 4.4.x.x // WinXP
					p = *(LPBYTE *)((LPBYTE)CIMM::GetSingleton()->LockIMCC( lpIC->hPrivate ) + 24 );
					if( !p ) break;
				...
				case IMEID_CHT_VER50: // 5.0.x.x // WinME
					p = *(LPBYTE *)( (LPBYTE)CIMM::GetSingleton()->LockIMCC( lpIC->hPrivate ) + 3 * 4 );
					if( !p ) break;
				...
				case IMEID_CHS_VER53: // 5.3.x.x // SCIME2k or MSPY3 (w/OfficeXP and Whistler)
					p = *(LPBYTE *)((LPBYTE)CIMM::GetSingleton()->LockIMCC( lpIC->hPrivate ) + 4);
					if( !p ) break;
				...
				case IMEID_CHS_VER41:
					{
						...
						p = *(LPBYTE *)((LPBYTE)CIMM::GetSingleton()->LockIMCC( lpIC->hPrivate ) + nOffset * 4);
						if( !p ) break;
				...
				case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k, Win2k, WinME, etc)
					{
						...
						p = *(LPBYTE *)((LPBYTE)CIMM::GetSingleton()->LockIMCC( lpIC->hPrivate ) + 1*4 + 1*4 + 6*4);
						if( !p ) break;
		...
		if( lpIC ) {
			CIMM::GetSingleton()->UnlockIMCC(lpIC->hPrivate);
		}
		CIMM::GetSingleton()->UnlockIMC(hImc);
